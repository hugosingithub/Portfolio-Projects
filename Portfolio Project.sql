...
Select location, population, max(total_cases)as HighestInfectionCount, max((total_cases/population)) as HighestInfectionPercentage
from [Portfolio Project]..CovidDeaths
...
Select date, sum(new_cases) as DailyTotalCases, sum(cast(new_deaths as int)) as DailyTotalDeaths, sum(cast(new_deaths as int))/sum(new_cases) as DailyDeathRate
from [Portfolio Project]..CovidDeaths
Where continent is not null
Group by date
order by date
...
Select sum(new_cases) as DailyTotalCases, sum(cast(new_deaths as int)) as DailyTotalDeaths, sum(cast(new_deaths as int))/sum(new_cases) as DailyDeathRate
from [Portfolio Project]..CovidDeaths
Where continent is not null

--Look at Total Cases vs Tatal Deaths in % form (Death Rate)
Select vac.location, vac.date, dea.population, vac.new_vaccinations, (vac.new_vaccinations/dea.population) as VaccinationRate
